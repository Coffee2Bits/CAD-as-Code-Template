...
`postCreateCommand` / `postStartCommand` in [`devcontainer.json`](https://github.com/Coffee2Bits/CAD-as-Code-Template/blob/main/.devcontainer/devcontainer.json) run `uv sync` (which brings in `ocp-viewer-mcp`) and, on start, `post-start.sh` (which runs `just view` for the startup OCP model and always runs `start-docs.sh` for the Docusaurus dev server). The launchers live under `.cursor/` and resolve paths relative to the workspace.
